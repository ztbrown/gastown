...
func TestParallelReadySteps(t *testing.T) {
	// Skip: mol-witness-patrol.formula.toml uses "Linear, Deacon-style" execution
	// as documented in its description: "No dynamic arms. No fanout gates."
	// The parallel=true flag is not set on any steps in this formula.
	// This test was written for a parallel execution model that was never implemented.
	t.Skip("Skipping: witness patrol formula uses linear execution, not parallel")
